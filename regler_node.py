...
class regelungs_node(Node):
    ...
    def timer_callback(self):
        if((time.time_ns()- self.last_calculation_time) > 5e9 
    ...
    def target_position_callback(self, msg):
        ...
        self.target_position['x'] = msg.target_position_x 
        self.target_position['y'] = msg.target_position_y 
        self.target_position['z'] = msg.target_position_z 
    ...
    def sort(self, oldest_object):
        ...
        if self.state_machine.current_state == StateMachine.picked_up:
            if oldest_object['class'] == 'cat':
                ...
                self.target_position = self.default_pos
            self.regler()
        elif self.state_machine.current_state == StateMachine.over_box:
            self.gripper_is_activated = False 
            robot_cmd = RobotCmd()
            robot_cmd.activate_gripper = self.gripper_is_activated
            self.robot_command_pub.publish(robot_cmd) 
    ...
    def update_state(self):
        ...
        if self.emergency:
            ...
            return
        elif self.target_position['x'] is None:
            self.state_machine.idling()
        elif (abs(self.robot_pos['x'] - self.box_cat['x']) < self.controlling_tolerance and
              abs(self.robot_pos['y'] - self.box_cat['y']) < self.controlling_tolerance) or (
             abs(self.robot_pos['x'] - self.box_unicorn['x']) < self.controlling_tolerance and
              abs(self.robot_pos['y'] - self.box_unicorn['y']) < self.controlling_tolerance) and \
            ...
            return
             ...
             ((abs(self.box_cat['x'] - self.target_position['x']) > self.controlling_tolerance or
              abs(self.box_cat['y'] - self.target_position['y']) > self.controlling_tolerance) or
              (abs(self.box_unicorn['x'] - self.target_position['x']) > self.controlling_tolerance or
              abs(self.box_unicorn['y'] - self.target_position['y']) > self.controlling_tolerance)) and \
            ...
            return
              ...
              (abs(self.ready_to_pick_up_z - self.robot_pos['z']) < self.controlling_tolerance or
            ...
            if not self.gripper_is_activated:
                self.gripper_is_activated = True
                robot_cmd = RobotCmd()
                robot_cmd.activate_gripper = self.gripper_is_activated
                self.robot_command_pub.publish(robot_cmd)
            ...
            return
              ...
              not self.gripper_is_activated or self.user_target):
            self.state_machine.move_to_object()
            ...
            return
              ...
              abs(self.target_position['y'] - self.robot_pos['y']) < self.controlling_tolerance and
              abs(self.target_position['z'] - self.robot_pos['z']) < self.controlling_tolerance and
              not self.oldest_object['index'] or not self.user_target):
            self.state_machine.idling()
